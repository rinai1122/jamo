"""Train V5: first-occurrence canonicalization, v4b recipe (pre-LN + warmup+cosine)."""
import sys
import io
import math
import time
...
def main():
    ...
    print(f"Training v5 (first-occurrence canon) for {STEPS} steps (batch={BATCH}, seq={SEQ_LEN})")
    t0 = time.time()
    ...
    torch.save(model.state_dict(), "jamo_nn_v5.pth")
    print(f"Saved jamo_nn_v5.pth ({time.time()-t0:.0f}s total)")
